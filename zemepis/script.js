var map = L.map('map', {
    ...
    // Move the call to the addEventListener() method outside of the then() callback
    const submitButton = document.getElementById('submitGuess');
    submitButton.addEventListener('click', submitGuess);
    
    // Declare the poiList variable using the let keyword
    let poiList;
    ...
          return [entry[0], [entry[1].latitude, entry[1].longitude]];
        });
        setNewPoi();
      })
